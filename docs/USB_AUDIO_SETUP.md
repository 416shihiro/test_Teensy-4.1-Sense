...
Teensy USBケーブル 1本
  ...
  └─ COMポート（115200, DATA **100Hz**）  → serial_hub.py → M4L + visualizer
```
...
`kPrintIntervalMs = 10` · IMU サンプル（10ms）と同期。M4L / viz 共通。
...
| 明日 | `plugin~` → piezo/mic チェーン接続、100Hz UDP 併用、レベル調整 |
...
- **IMU + エンベロープ + Serial**: 別スレッド的に並行、**100Hz**（~15 KB/s 程度）でも USB Serial は余裕
